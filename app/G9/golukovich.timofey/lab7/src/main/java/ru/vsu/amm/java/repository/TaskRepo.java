...
public class TaskRepo implements CrudRepo<TaskEntity> {
    ...
    public Optional<TaskEntity> getById(int id) throws SQLException {
        final String query = "SELECT task_id, employee_id, hotel_room_id, manager_id, description, created_at, updated_at FROM task WHERE task_id = ?";
    ...
    public List<TaskEntity> getAll() throws SQLException {
        final String query = "SELECT task_id, employee_od, hotel_room_id, manager_id, description, created_at, updated_at FROM task";
        var connection = dataSource.getConnection();
    ...
    public List<TaskEntity> getAllByEmployeeId(int employeeId) throws SQLException {
        final String query = """
                SELECT task_id, employee_id, hotel_room_id, manager_id, description, created_at, updated_at
                FROM task
                WHERE employee_id = ?""";
        var connection = dataSource.getConnection();
    ...
    public void update(TaskEntity entity) throws SQLException {
        final String query = """
                UPDATE task SET employee_id = ?, hotel_room_id = ?, manager_id = ?, description = ?, created_at = ?, updated_at = ?
                WHERE task_id = ?""";
        var connection = dataSource.getConnection();
    ...
    public TaskEntity save(TaskEntity entity) throws SQLException {
        final String query = """
                INSERT INTO task (employee_id, hotel_room_id, manager_id, description, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?)""";
        var connection = dataSource.getConnection();
        ...
        return configureTaskEntityFromResultSet(preparedStatement.getResultSet());
    }
    ...
    private void setPreparedStatement(PreparedStatement preparedStatement, TaskEntity entity) throws SQLException {
        ...
        if (entity.getManagerId() == null) {
            preparedStatement.setNull(3, entity.getManagerId());
        } else {
